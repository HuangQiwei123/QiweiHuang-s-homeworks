...
class TestEdgeCases:
    ...
    def test_negative_cn_raises(self):
        with pytest.raises(ValueError):
            calculate_runoff(50.0, -1.0)

    def test_zero_cn_all_infiltrates(self):
        assert calculate_runoff(50.0, 0.0) == 0.0

    def test_cn_above_100_raises(self):
        with pytest.raises(ValueError):
            calculate_runoff(50.0, 101.0)
    ...
    def test_vectorized_inputs(self):
        ...
        p = np.array([0.0, 50.0, 50.0])
        cn = np.array([80.0, 0.0, 100.0])
        q = calculate_runoff(p, cn)
        assert np.allclose(q, [0.0, 0.0, 50.0])
