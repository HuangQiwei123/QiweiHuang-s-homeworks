...
REFRESH_INTERVAL_SECONDS = int(os.environ.get("RAINFALL_REFRESH_SECONDS", "300"))
ALERT_LOG_DEDUPE_SECONDS = int(os.environ.get("RAINFALL_ALERT_DEDUPE_SECONDS", "900"))
...
def rainfall_range_label(level: str) -> str:
    labels = {
        "green": "< 10 mm/h",
        "yellow": "10-20 mm/h",
        "red": ">= 20 mm/h",
...
def extract_rainfall_mmh(rain_block: Any) -> float:
    """Extract rainfall intensity from OpenWeather rain data."""
    if not isinstance(rain_block, dict):
        return 0.0
    if "1h" in rain_block:
        return float(rain_block["1h"])
    if "3h" in rain_block:
        return float(rain_block["3h"]) / 3.0
    return 0.0
...
def fetch_weather(city_en: str, lat: float, lon: float) -> dict[str, Any]:
    ...
    try:
        ...
        rain_mmh = extract_rainfall_mmh(data.get("rain", {}))
...
def should_log_alert(city_en: str, level: str) -> bool:
    """Avoid repeatedly logging the same city/level alert on every refresh."""
    now = time.time()
    key = f"{city_en}:{level}"
    guard = st.session_state.setdefault("alert_log_guard", {})
    last_logged = guard.get(key, 0.0)
    if now - last_logged < ALERT_LOG_DEDUPE_SECONDS:
        return False
    guard[key] = now
    return True


def configure_auto_refresh(enabled: bool) -> None:
    """Request a dashboard rerun at the experiment's 5-minute cadence."""
    if not enabled:
        return
    if st_autorefresh is not None:
        st_autorefresh(interval=REFRESH_INTERVAL_SECONDS * 1000, key="rainfall_auto_refresh")
    else:
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.caption(f"Refresh interval: **{REFRESH_INTERVAL_SECONDS // 60} min**")
        st.caption(
            f"{'🔶 Simulation Mode' if USE_SIMULATION else '🟢 Live API Mode'}"
        )

    configure_auto_refresh(auto_refresh)
        ...
        with st.spinner("Fetching current rainfall data..."):
            ...
            for city_en, (city_cn, lat, lon) in CITIES.items():
                ...
                if data["rainfall_mmh"] >= ALERT_THRESHOLD:
                    alerts.append(data)
                    if should_log_alert(city_en, cls["level"]):
            ...
            st.session_state.next_refresh_at = (
                time.time() + REFRESH_INTERVAL_SECONDS if auto_refresh else float("inf")
            )
    ...
    if errors:
        failed = ", ".join(r["city_cn"] for r in errors)
        st.warning(f"Weather API request failed for {failed}; displayed values defaulted to 0.")
